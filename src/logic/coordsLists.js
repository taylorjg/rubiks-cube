...
export const makeAllCoordsList = cubeSize =>
  Array.from(allCoordsGenerator(cubeSize))

export const pitchSliceCoordsList = (allCoordsList, xSlice) =>
  allCoordsList.filter(([x]) => x === xSlice)
export const yawSliceCoordsList = (allCoordsList, ySlice) =>
  allCoordsList.filter(([, y]) => y === ySlice)
export const rollSliceCoordsList = (allCoordsList, zSlice) =>
  allCoordsList.filter(([, , z]) => z === zSlice)
